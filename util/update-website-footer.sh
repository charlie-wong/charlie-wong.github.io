...
jsKey='wRepositoryHome'; jsVal='https://github.com/xwlc/xwlc.github.io'
echo "const ${jsKey} = '${jsVal}';"  > "${FooterJS}"

jsKey='wRepoUpdateTime'; jsVal="$(date +%Y-%m-%d)"
echo "const ${jsKey} = '${jsVal}';" >> "${FooterJS}"

jsKey='wRepoCommitHash'; jsVal="$(git rev-parse --short HEAD)"
echo "const ${jsKey} = '${jsVal}';" >> "${FooterJS}"
